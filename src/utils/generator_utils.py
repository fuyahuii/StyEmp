...
def encode_example_calibrate(example, tokenizer, roberta_tokenizer):
    max_len=128
    max_annot_len = {
        "personality": 128,
        "empathy": 128,
    ...
    candidate=example['candidates'].split("\n")
    for i in range(len(candidate)):
        candidate[i]=tokenizer.eos_token + candidate[i] + tokenizer.eos_token
    ...
    encoded_src = tokenizer(src)
    encoded_tgt = tokenizer(tgt)
    encoded_candidates = [tokenizer(item) for item in candidate]

    src_ids = encoded_src.input_ids
    tgt_ids = encoded_tgt.input_ids
    candidates_ids=[item.input_ids for item in encoded_candidates]
    ...
    srccondi_ids=[src_ids+item for item in candidates_ids]
