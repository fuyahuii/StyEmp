...
def custom_evalutions(generation,result_path):
	...
	parser.add_argument("--Emotion_model_path", type=str, default='src/custom_eval/Pretrained/emotion_roberta-base_epochs15_lr2e-05_dropout0.1_warmup0_seed12/model.pt', help="path to emotion classification model")
	parser.add_argument("--ER_model_path", type=str,default='src/custom_eval/Pretrained/emotion_react_roberta-base_epochs4_lr2e-05_dropout0.1_warmup0_seed12/model.pt', help="path to emotion react model")
	parser.add_argument("--IP_model_path", type=str, default='src/custom_eval/Pretrained/interpretations_studio-ousia_luke-base_epochs4_lr2e-05_dropout0.1_warmup0_seed12/model.pt',help="path to interpretation model")
	parser.add_argument("--EX_model_path", type=str, default='src/custom_eval/Pretrained/explorations_roberta-base_epochs5_lr2e-05_dropout0.1_warmup0_seed12/model.pt', help="path to exploration model")
	parser.add_argument("--EI_model_path", type=str, default='src/custom_eval/Pretrained/intent_studio-ousia_luke-base_epochs7_lr2e-05_dropout0.1_warmup0_seed12/model1.pt', help="path to empathetic intent model")
	parser.add_argument("--Big5_model_path", type=str, default='src/custom_eval/Pretrained/big5_preprocessing_hidden16_mse_lr2e-05_pretrainstudio-ousia_luke-base_dropout0.2_warmup100/model.pt', help="path to personality model")
	parser.add_argument("--MBTI_E_model_path", type=str, default='src/custom_eval/Pretrained/MBTI_introverted_regression_mse_hidden16_lr1e-05_batch_size120_pretrainstudio-ousia_luke-base_dropout0.1_warmup100/model.pt', help="path to MBTI Extroversion model")
	parser.add_argument("--MBTI_thinking_model_path", type=str, default='src/custom_eval/Pretrained/MBTI_thinking_regression_mse_hidden16_lr1e-05_batch_size128_pretrainstudio-ousia_luke-base_dropout0.1_warmup100/model.pt', help="path to MBTI Thinking model")
	...
	# output_path= os.path.join('src/custom_eval/output/calibration',input_file)
	...
	# generated = [item.lower() for item in generated]
	# true = [item.lower() for item in true]

	print(generated[0])
	print(true[0])
	assert len(generated)==len(true)

	# personality_scores=pd.read_csv('src/custom_eval/test_dataset4.csv')
	personality_scores=pd.read_csv('dataset/retrieved_dataset/dataset4/test.csv')
	...
	output_file=open(os.path.join(output_path,'generated_annotated_p0.8_t0.7_1.csv'),'w')
