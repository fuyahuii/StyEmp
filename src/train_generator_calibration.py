...
class CustomTrainer(Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.num_candidate=kwargs.pop("num_candidate",5)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if model.training:    
            ...
            #     original_loss = original_loss[:batch_size*(self.num_candidate+1)]
...
def main():
    ...
    # save_variable="style_both_batch_size_64_lr_5e-05_warmup_0_speaker_25_empathy_25_addcontext_False_concontext_True_diffencoder_True" #folder of the best checkpoint after train_generator.py
    ...
    result_input_path=os.path.join(args.result_input_path,save_variable,f"num_candidate_{args.num_candidate}")
    result_temp_path=os.path.join(args.temp_path,save_variable,f"num_candidate_{args.num_candidate}")
    ...
    if args.stylizeEncoder:
        dataset_path=os.path.join(data_path, f"num_candidate_{args.num_candidate}")
        ...
        if local_rank <= 0 and not os.path.exists(dataset_path):
            ...
            conversation_train_new = conversation_train.loc[conversation_train.index.repeat(args.num_candidate+1)]
            ...
            update_indices = np.delete(all_indices, np.arange(0, total_length, args.num_candidate + 1))
